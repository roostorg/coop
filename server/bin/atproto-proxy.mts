...
async function resolveActor(actor: string): Promise<string> {
  ...
  if (!res.ok)
    throw new Error(`Failed to resolve actor ${actor}: ${res.status}`);
...
function renderEmbed(embed: BskyEmbed, postUrl: string): string {
  ...
  if (type === 'app.bsky.embed.images#view' && embed.images?.length) {
    const imgs = embed.images
      ...
      .join('\n    ');
    const gridClass =
      embed.images.length === 1 ? 'embed-images single' : 'embed-images';
    return `\n  <div class="${gridClass}">\n    ${imgs}\n  </div>`;
  }
  ...
  if (type === 'app.bsky.embed.recordWithMedia#view') {
    ...
    const recordPart = embed.record
      ? renderEmbed(
          { $type: 'app.bsky.embed.record#view', record: embed.record },
          postUrl,
        )
      : '';
...
<head>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    img { filter: var(--img-filter, none); cursor: zoom-in; }
    img:hover { filter: none; transition: filter 0.2s ease; }
    .card {
