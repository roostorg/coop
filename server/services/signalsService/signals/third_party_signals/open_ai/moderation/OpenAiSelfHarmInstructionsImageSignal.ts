 ...
 * (0..1).
 */
export default class OpenAiSelfHarmInstructionsImageSignal extends SignalBase<
  ScalarTypes['IMAGE'],
...
> {
  ...
  override get id() {
    return { type: SignalType.OPEN_AI_SELF_HARM_INSTRUCTIONS_IMAGE_MODEL };
  }

  override get displayName() {
    return 'OpenAI Self-Harm Instructions Image score';
  }

  override get description() {
    return `OpenAI's model that detects content that encourages performing acts of self-harm, such as suicide, cutting, and eating disorders, or that gives instructions or advice on how to commit such acts. Scored against the image.
  ...
  /**
   * Fetches the omni-moderation `self-harm/instructions` score for the image
