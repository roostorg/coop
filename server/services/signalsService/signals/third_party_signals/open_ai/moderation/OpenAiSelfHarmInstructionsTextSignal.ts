 ...
 */
export default class OpenAiSelfHarmInstructionsTextSignal extends SignalBase<
  ScalarTypes['STRING'],
...
> {
  ...
  override get id() {
    return { type: SignalType.OPEN_AI_SELF_HARM_INSTRUCTIONS_TEXT_MODEL };
  }

  override get displayName() {
    return 'OpenAI Self-Harm Instructions Text score';
  }

  override get description() {
    return `OpenAI's model that detects content that encourages performing acts of self-harm, such as suicide, cutting, and eating disorders, or that gives instructions or advice on how to commit such acts.
  ...
  /**
   * Fetches the omni-moderation `self-harm/instructions` score for the text
