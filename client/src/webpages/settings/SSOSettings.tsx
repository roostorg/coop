...
export default function SSOSettings() {
  ...
  const [updateSSOSettings, { loading: updateLoading }] =
    useGQLUpdateSsoSettingsMutation();
  ...
  const currentMethod = samlEnabled
    ? 'SAML'
    : oidcEnabled
      ? 'OIDC'
      : 'Password';
  ...
  const samlCallbackUri =
    callbackData?.getSSOCallbackUrls.samlCallbackUrl ?? '';
  const samlIssuer = callbackData?.getSSOCallbackUrls.samlIssuer ?? '';
  const oidcCallbackUri =
    callbackData?.getSSOCallbackUrls.oidcCallbackUrl ?? '';

  const stringIsAValidUrl = (s: string) => URL.canParse(s);

  const isSamlFormValid = ssoUrl.length > 0 && ssoCert.length > 0;
  // When OIDC is already active, the secret may be left blank to keep the stored one.
  const isOidcFormValid =
    issuerUrl.length > 0 &&
    clientId.length > 0 &&
    (clientSecret.length > 0 || oidcEnabled);
  const isFormValid = activeTab === 'SAML' ? isSamlFormValid : isOidcFormValid;

  const handleSaveSaml = (closeDialog = false) => {
    // Trim before validating *and* saving: URL.canParse already tolerates
    // surrounding whitespace (the URL parser strips it), so a padded paste would
    // otherwise pass validation and be persisted verbatim.
    const trimmedSsoUrl = ssoUrl.trim();
    if (!stringIsAValidUrl(trimmedSsoUrl)) {
      toast.error('SSO URL is not a valid URL');
      return;
    }
    updateSSOSettings({
      variables: { input: { saml: { ssoUrl: trimmedSsoUrl, ssoCert } } },
      refetchQueries: ['GetSSOCredentials'],
      onCompleted: () => {
        toast.success(
          isSwitching ? 'Switched to SAML' : 'SAML credentials updated',
        );
  ...
  const handleSaveOidc = (closeDialog = false) => {
    ...
    updateSSOSettings({
      ...
      onCompleted: () => {
        toast.success(
          isSwitching ? 'Switched to OIDC' : 'OIDC credentials updated',
        );
        if (closeDialog) setShowSwitchDialog(false);
      },
      onError: (e) => {
    ...
    <div className="flex flex-col w-3/5 gap-4 text-start">
      ...
      {activeTab === 'SAML' && (
        <>
          ...
          <Text size="SM">
            Enter this information into your identity provider's "Service
          ...
          <div className="flex flex-col gap-2">
            ...
            <Text id="SsoCert" size="SM">
              ...
              organization when users attempt to log in via SSO. Please ensure
              this certificate matches the one provided by your identity
              provider.
            </Text>
          </div>
          <Textarea
      ...
      {activeTab === 'OIDC' && (
        <>
          <Heading>SSO Configuration</Heading>
          <Text size="SM">
            Enter this information into your identity provider's application
            setup.
          </Text>
          <div className="flex flex-col gap-2 mb-8">
          ...
          <div className="flex flex-col gap-2">
            ...
            <Input
              ...
              value={clientSecret}
              placeholder={
                oidcEnabled ? '••••••••  (enter new value to change)' : ''
              }
      ...
      {showSwitchDialog && (
        <Dialog open onOpenChange={setShowSwitchDialog}>
