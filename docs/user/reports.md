...
If `reportedForReason.csam` is `true`, the job is routed directly to the NCMEC queue rather than going through the normal routing rule evaluation. See [Child Safety Reporting](child-safety.md) for details.
