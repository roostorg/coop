...
| Status                  | Meaning                                                                                                                                                   |
| :---------------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `200 OK`                | Request succeeded; response body contains data (e.g. `GET /api/v1/policies/`)                                                                             |
...
| `401 / 403`             | Authentication failure — API key is missing, invalid, or expired                                                                                          |
| `429 Too Many Requests` | Rate limit exceeded. Contact support to request a quota increase                                                                                          |
| `500 / 503`             | Internal server error — typically transient; safe to retry                                                                                                |
| `502 / 504`             | Gateway or dependency error — an upstream service is unavailable; retry or contact support if persistent                                                  |
...
| `requestId` | Correlation ID for tracing — include this when contacting support |
