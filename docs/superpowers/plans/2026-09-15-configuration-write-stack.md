...
**Architecture:** ModerationConfigService owns item types; ManualReviewToolService owns hidden-field configuration. GraphQL and REST construct transaction-bound service instances and invalidate the original item-type cache after commit.
...
## 1. Item-type service safeguards
...
- [x] Fold #1271's cache invalidation support into #1272; remove standalone transaction-plumbing tests.
- [x] Inject transactions through service constructors; leave the retry helper unchanged.
...
## 2. GraphQL and editor integration
...
- [x] Construct both services with the same transaction; expose useful domain errors through existing GraphQL error patterns.
...
## 3. Policy/action safeguards and REST creates and updates
...
- [x] Publish PRs #1272–#1275 as a four-PR GitHub stack based on `main`.

## Publishing and verification limits

The read API branch and PR #1144 remain untouched. The chain is
`main` → `safety/item-type-configuration` →
...
All four draft PRs use branches in `roostorg/coop`. Database-backed service tests
