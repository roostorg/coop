...
| **Hash Bank for Reported Media**         | Media from each CyberTip accepted by NCMEC is added to this hash bank so it can be matched if it is uploaded again. Adding runs in the background (`ReportedMediaBankingWorker`), so it never blocks or fails the CyberTip. Only applies to [production submissions](#test-vs-production-submissions). |
